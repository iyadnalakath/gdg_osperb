...
from .serializer import LedgerHeadSerializer,LedgerSerializer,LedgerEntrySerializer,ContraSerializer
...
class ContraViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        try:
            if request.user.role in ["admin"]:
                queryset = Contra.objects.all()
                serializer = ContraSerializer(queryset, many=True)
                return Response(serializer.data)
            else:
                raise PermissionDenied("You are not allowed to view contra.")
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            contra = self.get_object()
            if request.user.role in ["admin"]:
                serializer = ContraSerializer(contra)
                return Response(serializer.data, status=status.HTTP_200_OK)
            else:
                raise PermissionDenied("You are not allowed to view this object.")
        except PermissionDenied as e:
            ...
            return Response({'error': 'Contra not found.'}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        
    def create(self, request, *args, **kwargs):
        try:
            serializer = ContraSerializer(data=request.data)
            if serializer.is_valid():
                if self.request.user.role == "admin":
                    serializer.save()
                    return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            if request.user.role == "admin":
                contra.delete()
                return Response({'response': 'contra deleted successfully.'}, status=status.HTTP_204_NO_CONTENT)
            else:
                raise PermissionDenied("You are not allowed to delete this object.")
        except ValidationError as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, *args, **kwargs):
        try:
            contra = self.get_object()
            serializer = ContraSerializer(contra, data=request.data, partial=True)
            if serializer.is_valid():
                if request.user.role == "admin":
                    serializer.save()
                    return Response(serializer.data, status=status.HTTP_200_OK)
